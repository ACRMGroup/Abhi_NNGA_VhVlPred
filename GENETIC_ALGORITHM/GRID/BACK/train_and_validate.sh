...
then
   ...
   echo "7. Number of bins - Optional parameter"
   echo "8. Bin option: -binT or -binTV - Optional parameter"
   echo
   exit 0
fi
...
then
   ...
   command="./train_and_validate.exe -in $inputFilename -num $geneNumber -int all_interface_positions.dat -pdb $ABINTERFACE/Fv_Fab_pdb.lst -tor $ABINTERFACE/INTERFACE_ANGLE/torsion_angles.txt -type $geneType -odir $outputDirectory -gen $generation $binOption"

else

   command="./train_and_validate.exe -in $inputFilename -num $geneNumber -int all_interface_positions.dat -pdb $ABINTERFACE/Fv_Fab_pdb.lst -tor $ABINTERFACE/INTERFACE_ANGLE/torsion_angles.txt -type $geneType -odir $outputDirectory -gen $generation"
