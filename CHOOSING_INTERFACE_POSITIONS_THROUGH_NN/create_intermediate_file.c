...
static KABATLIST *kabatNumberedListFirstPtr=NULL,
       *p=NULL,*prev=NULL; /* Pointers used to release the memory allocated to the linked list. */

static char **interfacePositionsList=NULL;

static int numberOfInterfacePositions=0,
	   i=0;

static char *residuesAtInterfacePositions=NULL;

static float **interfaceResiduesMatrix=NULL;

static float interfaceAngle=-1;

static FILE *wfp=NULL;

static BOOL returnValue=0;
					  ...
					  char *residuesAtInterfacePositions)   /* Array to which residues must be written. */
{
   ...
   while(i < numberOfInterfacePositions)
   {
      ...
      while( p && strcmp(p->position,interfacePositionsList[i]) )
	 p=p->next;
...
} /* End of function "extract_interface_angle" */


void free_memory(int numberOfArrays)
{
   int i=0;

   for(i=0;i<numberOfInterfacePositions;i++)
   {
      if(interfacePositionsList[i])
      {
	 free(interfacePositionsList[i]);
      }

      if( (numberOfArrays > 1) && interfaceResiduesMatrix[i] )
      {
   ...
   if( (numberOfArrays > 1) && interfaceResiduesMatrix)
   {
...
int main(int argc,char **argv)
{
   ...
   if(! returnValue)
   {
      printf("\nOne or more positions in the interface positions list was not");
      printf("found in the structure \"%s\". Exiting program\n\n",pdbCode);
   ...
   /* Step 6: Make correspondence between the amino acids in the interface positions and
   ...
   */
   ...
   for(i=0;i<numberOfInterfacePositions;i++)
   {
      interfaceResiduesMatrix[i]=NULL;
   }
   ...
   /* Step 7: Obtain the interface interface angle from the respective file */

   interfaceAngle=extract_interface_angle(interfaceAngleFilename,pdbCode);

   /* Step 8: Write the information gathered from the above steps into the output file. */
   ...
   /* Step 9: Release memory allocated to the variables */
